...
namespace ReleaseDeployerService.Core
{
    public class GithubV3AssetDownloader : IAssetDownloader
    {
        ...
        public async Task<Deployable?> DownloadAsync()
        {
            ...
            {
                client.DefaultRequestHeaders.Add("Authorization", $"Bearer {_reader.GetGitToken()}");
                client.DefaultRequestHeaders.Add("Accept", "application/vnd.github.v3+json");
                client.DefaultRequestHeaders.Add("User-Agent", _reader.GetGitRepo());
                var responseMessage = await client.GetAsync($"https://api.github.com/repos/{_reader.GetGitUserName()}/{_reader.GetGitRepo()}/releases");
                ...
                client.DefaultRequestHeaders.Add("Accept", deployableType == DeployableType.Asset ? "application/octet-stream" : "application/vnd.github.V3.raw");
                responseMessage = await client.GetAsync(downloadUri);
